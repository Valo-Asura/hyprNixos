...
PanelWindow {
    ...
    function setColorPreset(name) {
        ...
        // Re-apply when selecting the same preset again.
        if (name && name === activeColorPreset) {
            applyColorPreset();
        }
    }
    ...
    FileView {
        ...
        onLoaded: {
            ...
            // Apply active preset on startup so colors.json stays in sync after restarts.
            Qt.callLater(function() {
                if (wallpaperConfig.adapter.activeColorPreset) {
                    wallpaper.applyColorPreset();
                }
            });
        }
