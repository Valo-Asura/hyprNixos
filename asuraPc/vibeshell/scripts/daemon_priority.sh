...
# LiteLLM Proxy. Vibeshell talks to Ollama directly by default; keep LiteLLM
# opt-in so boot stays fast and stale cloud keys cannot break local chat.
if [ "${VIBESHELL_ENABLE_LITELLM:-0}" != "1" ]; then
	echo "LiteLLM disabled by default; set VIBESHELL_ENABLE_LITELLM=1 to start it"
	...
	if [ -f "$CONFIG_PATH" ]; then
		LOG_DIR="${XDG_CACHE_HOME:-$HOME/.cache}/vibeshell"
		mkdir -p "$LOG_DIR"
		chmod 700 "$LOG_DIR"
		...
		if [ -n "${VIBESHELL_WAIT_LITELLM:-}" ] && command -v curl >/dev/null 2>&1; then
