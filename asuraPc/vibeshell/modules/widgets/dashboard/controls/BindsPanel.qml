...
Item {
    ...
    property string currentCategory: "vibeshell"
    ...
    property bool isEditingVibeshell: false
    ...
    function openEditDialog(bind, index, isVibeshell) {
        root.editingIndex = index;
        root.editingBind = bind;
        root.isEditingVibeshell = isVibeshell;

        // Initialize edit form state
        if (isVibeshell) {
            // Vibeshell binds still use old format (single key)
    ...
    function saveEdit() {
        ...
        if (root.isEditingVibeshell) {
            // Save vibeshell bind (still uses old format internally)
            const path = root.editingBind.path.split(".");
            // path = ["vibeshell", "dashboard"|"system", "bindName"]
            const section = path[1];
            const bindName = path[2];

            const adapter = Config.keybindsLoader.adapter;
            if (adapter && adapter.vibeshell && adapter.vibeshell[section] && adapter.vibeshell[section][bindName]) {
                // Use first key for vibeshell binds
                ...
                const bindObj = adapter.vibeshell[section][bindName];
    ...
    // Get vibeshell binds as a flat list
    function getVibeshellBinds() {
        const adapter = Config.keybindsLoader.adapter;
        if (!adapter || !adapter.vibeshell)
            return [];

        const binds = [];
        const vibeshell = adapter.vibeshell;
        ...
        if (vibeshell.dashboard) {
            ...
            for (const key of dashboardKeys) {
                if (vibeshell.dashboard[key]) {
                    binds.push({
                        ...
                        path: "vibeshell.dashboard." + key,
                        bind: vibeshell.dashboard[key]
                    });
                }
            }
        }
            ...
            if (vibeshell.system) {
                ...
                for (const key of systemKeys) {
                    if (vibeshell.system[key]) {
                        binds.push({
                            category: "System",
                        name: key.charAt(0).toUpperCase() + key.slice(1),
                        path: "vibeshell.system." + key,
                        bind: vibeshell.system[key]
                    });
                }
            }
        }
    ...
    Flickable {
        ...
        ColumnLayout {
            ...
            // Vibeshell binds view
            Repeater {
                id: vibeshellRepeater
                model: root.currentCategory === "vibeshell" ? root.getVibeshellBinds() : []
            ...
            Text {
                ...
                Layout.topMargin: 20
                visible: (root.currentCategory === "vibeshell" && vibeshellRepeater.count === 0) || (root.currentCategory === "custom" && customRepeater.count === 0)
                text: root.currentCategory === "vibeshell" ? "No Vibeshell binds configured" : "No custom binds configured"
                font.family: Config.theme.font
                font.pixelSize: Styling.fontSize(0)
            ...
            ColumnLayout {
                ...
                Item {
                    ...
                    RowLayout {
                        ...
                        StyledRect {
                            id: deleteButton
                            visible: !root.isEditingVibeshell && !root.isCreatingNew
                        ...
                        // Reset button (only for Vibeshell binds)
                        StyledRect {
                            id: resetButton
                            visible: root.isEditingVibeshell
                            ...
                            MouseArea {
                                ...
                                cursorShape: Qt.PointingHandCursor
                                onClicked: {
                                    if (root.isEditingVibeshell && root.editingBind) {
                                        const path = root.editingBind.path.split(".");
                                        // path = ["vibeshell", "dashboard"|"system", "bindName"]
                                        const section = path[1];
                                        const bindName = path[2];
                                        ...
                                        const defaultBind = Config.keybindsLoader.adapter.getVibeshellDefault(section, bindName);
                ...
                Item {
                    ...
                    ColumnLayout {
                        ...
                        // Bind name/info (for vibeshell binds only)
                        Text {
                            visible: root.isEditingVibeshell && root.editingBind !== null
                        ...
                        ColumnLayout {
                            ...
                            RowLayout {
                                ...
                                StyledRect {
                                    id: removeKeyBtn
                                    visible: root.editKeys.length > 1 && !root.isEditingVibeshell
                        ...
                        // =====================
                        // ACTIONS SECTION (custom binds & flags for vibeshell)
                        // =====================
                        ColumnLayout {
                            Layout.fillWidth: true
                            spacing: 8
                            // visible: !root.isEditingVibeshell - Removed to allow editing flags for Vibeshell binds
                            ...
                            RowLayout {
                                ...
                                // Page indicator
                                Text {
                                    visible: root.editActions.length > 1 && !root.isEditingVibeshell
                            ...
                            StyledRect {
                                ...
                                radius: Styling.radius(-2)
                                opacity: root.isEditingVibeshell ? 0.6 : 1.0
                                ...
                                TextInput {
                                    ...
                                    verticalAlignment: Text.AlignVCenter
                                    selectByMouse: true
                                    readOnly: root.isEditingVibeshell
                                    onTextChanged: {
                                        if (root.editActions.length > root.currentActionPage) {
                                            const currentAction = root.editActions[root.currentActionPage];
    ...
    component BindItem: StyledRect {
        ...
        property bool isVibeshell: true
        property bool isHovered: false
