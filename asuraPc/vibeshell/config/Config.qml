...
Singleton {
    ...
    property string configDir: (Quickshell.env("XDG_CONFIG_HOME") || (Quickshell.env("HOME") + "/.config")) + "/Vibeshell/config"
    property string keybindsPath: (Quickshell.env("XDG_CONFIG_HOME") || (Quickshell.env("HOME") + "/.config")) + "/Vibeshell/binds.json"
        ...
        adapter: JsonAdapter {
            ...
            property JsonObject idle: JsonObject {
                property JsonObject general: JsonObject {
                    ...
                    property string before_sleep_cmd: "vibeshell-lock-before-sleep"
                    property string after_sleep_cmd: "vibeshell screen on"
                }
                property list<var> listeners: [
                    {
                        "timeout": 150,
                        "onTimeout": "vibeshell brightness 10 -s",
                        "onResume": "vibeshell brightness -r"
                    },
                    {
                        "timeout": 300,
                        "onTimeout": "vibeshell-safe-lock"
                    },
                    {
                        "timeout": 330,
                        "onTimeout": "vibeshell screen off",
                        "onResume": "vibeshell screen on"
                    },
                    {
                        "timeout": 1800,
                        "onTimeout": "vibeshell screen off",
    ...
    function repairKeybinds() {
        ...
        try {
            ...
            // Ensure vibeshell structure exists
            if (!current.vibeshell) {
                current.vibeshell = {};
                needsUpdate = true;
            }
            if (!current.vibeshell.dashboard) {
                current.vibeshell.dashboard = {};
                needsUpdate = true;
            }
            if (!current.vibeshell.system) {
                current.vibeshell.system = {};
                needsUpdate = true;
            }

            // Get default binds from adapter
            const adapter = keybindsLoader.adapter;
            if (!adapter || !adapter.vibeshell) return;
            ...
            for (const key of dashboardKeys) {
                if (!current.vibeshell.dashboard[key] && adapter.vibeshell.dashboard && adapter.vibeshell.dashboard[key]) {
                    ...
                    current.vibeshell.dashboard[key] = createCleanBind(adapter.vibeshell.dashboard[key]);
                    needsUpdate = true;
                }
            }
            ...
            for (const key of systemKeys) {
                if (!current.vibeshell.system[key] && adapter.vibeshell.system && adapter.vibeshell.system[key]) {
                    console.log("Adding missing system bind:", key);
                    current.vibeshell.system[key] = createCleanBind(adapter.vibeshell.system[key]);
                    needsUpdate = true;
                }
            }
    ...
    FileView {
        ...
        adapter: JsonAdapter {
            property JsonObject vibeshell: JsonObject {
                property JsonObject dashboard: JsonObject {
                    ...
                    property JsonObject emoji: JsonObject {
                        ...
                        property string dispatcher: "exec"
                        property string argument: "vibeshell run dashboard-emoji"
                    }
                    property JsonObject notes: JsonObject {
                        property list<string> modifiers: ["SUPER"]
                        property string key: "N"
                        property string dispatcher: "exec"
                        property string argument: "vibeshell run dashboard-notes"
                    }
                    property JsonObject tmux: JsonObject {
                        property list<string> modifiers: ["SUPER"]
                        property string key: "T"
                        property string dispatcher: "exec"
                        property string argument: "vibeshell run dashboard-tmux"
                    }
                ...
                property JsonObject system: JsonObject {
                    ...
                    property JsonObject lockscreen: JsonObject {
                        ...
                        property string dispatcher: "exec"
                        property string argument: "vibeshell-safe-lock"
                        property string flags: ""
                    }
                    property JsonObject overview: JsonObject {
                        ...
                        property string dispatcher: "exec"
                        property string argument: "vibeshell run overview"
                        property string flags: ""
                    }
                    property JsonObject powermenu: JsonObject {
                        ...
                        property string dispatcher: "exec"
                        property string argument: "vibeshell run powermenu"
                        property string flags: ""
                    }
                    ...
                    property JsonObject lens: JsonObject {
                        ...
                        property string key: "A"
                        property string dispatcher: "exec"
                        property string argument: "vibeshell run lens"
                        property string flags: ""
                    }
                    property JsonObject reload: JsonObject {
                        ...
                        property string dispatcher: "exec"
                        property string argument: "vibeshell reload"
                        property string flags: ""
                    }
                    property JsonObject quit: JsonObject {
                        ...
                        property string key: "B"
                        property string dispatcher: "exec"
                        property string argument: "vibeshell quit"
                        property string flags: ""
                    }
                }
            }
            ...
            function getVibeshellDefault(section, key) {
                if (defaultVibeshellBinds[section] && defaultVibeshellBinds[section][key]) {
                    const bind = defaultVibeshellBinds[section][key];
                    return {
                        "modifiers": bind.modifiers || [],
                        "key": bind.key || "",
                        "dispatcher": bind.dispatcher || "",
            ...
            property list<var> custom: [
                ...
                // Brightness Controls
                {
                    ...
                    "actions": [
                        {
                            "dispatcher": "exec",
                            "argument": "vibeshell brightness +5",
                            "flags": "le",
                            "compositor": {
                                "type": "hyprland",
                                "layouts": []
    ...
    function migrateSystemIdle(config) {
        if (!config || !config.idle)
            return;

        if (config.idle.general && config.idle.general.before_sleep_cmd === "vibeshell-safe-lock") {
            config.idle.general.before_sleep_cmd = "vibeshell-lock-before-sleep";
        }

        if (!Array.isArray(config.idle.listeners))
            return;

        var hasLateScreenOff = false;
        var hasSuspend = false;
        for (var i = 0; i < config.idle.listeners.length; i++) {
            var listener = config.idle.listeners[i];
            if (listener.onTimeout === "vibeshell screen off" && listener.timeout >= 1800) {
                hasLateScreenOff = true;
            }
            if (listener.onTimeout === "vibeshell suspend") {
                hasSuspend = true;
                if (listener.timeout < 3600)
                    listener.timeout = 3600;
            }
        }

        if (!hasLateScreenOff && hasSuspend) {
            config.idle.listeners.splice(config.idle.listeners.length - 1, 0, {
                "timeout": 1800,
                "onTimeout": "vibeshell screen off",
                "onResume": "vibeshell screen on"
            });
